...
Beim **Lesen** wird `schemaVersion` strikt auf Gleichheit geprüft. Eine
fehlende, ältere oder zukünftige Version ist ein einzelner ungültiger Eintrag
und wird übersprungen, nicht als aktuelle umgedeutet – sonst erschienen fehlende
Felder eines fremden Schemas still als `0` oder „unbekannt“. Beim Schreiben wird
die Version dagegen vergeben, nicht geprüft.
...
„Best effort“ heißt ausdrücklich auch **begrenzt**: jeder Historienzugriff –
Write wie Read – läuft gegen `FEED_RUN_HISTORY_TIMEOUT_MS` (3 Sekunden). Ein
Speicher, der gar nicht antwortet, ist etwas anderes als einer, der einen Fehler
meldet; ohne Frist bliebe `finish()` hängen, obwohl der Lauf fachlich fertig
ist, und die Health-Antwort käme nie an. Der Zeitgeber wird auf jedem
Abschlussweg abgeräumt, und eine verspätete Ablehnung des überholten Zugriffs
bekommt einen eigenen Handler – die Historie darf den Prozess nicht mit einer
unbehandelten Ablehnung beenden. Frist und Zeitsteuerung sind injizierbar.
