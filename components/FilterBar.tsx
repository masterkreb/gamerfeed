            ...
            <div className="flex items-center gap-2">
                ...
                `}
                    >
                        ...
                        <span className="hidden sm:inline">{isCurrentSearchSaved ? t('filter.search.saved') : t('filter.search.save')}</span>
                    </button>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                <div>
                    ...
                    <select id="time-filter" value={timeFilter} onChange={(e) => setTimeFilter(e.target.value as TimeFilter)} className={`${baseSelectClasses} ${timeFilter !== 'all' ? activeFilterClasses : ''}`}>
                        ...
                        <option value="7d">{t('filter.time.7d')}</option>
                    </select>
                </div>

                <div>
                    <label className="block text-xs font-medium text-slate-500 dark:text-zinc-400 mb-1">{t('filter.language.label')}</label>
                ...
                <div>
                    ...
                    >
                        <option value="all">
                            {languageFilter === 'all' && t('filter.source.allSources', { count: sources.length })}
                            {languageFilter === 'de' && t('filter.source.allGerman', { count: germanSources.length })}
                            {languageFilter === 'en' && t('filter.source.allEnglish', { count: englishSources.length })}
                        </option>
                        {languageFilter === 'all' ? (
                            <>
                                {germanSources.length > 0 && (
                                    <optgroup label={t('filter.source.germanSources')}>
                                ...
                                {englishSources.length > 0 && (
                                    <optgroup label={t('filter.source.englishSources')}>
                            ...
                            <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                    <p className="text-xs font-semibold text-amber-800 dark:text-amber-300">
                                        {t('filter.noMatchWarning')}
                                    </p>
                                    <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
                                        {t('filter.noMatchHint')}
                                    </p>
                                </div>
                            </div>
                        </div>
