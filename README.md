...
3.  **Datencache (Vercel KV)**: Ein extrem schneller In-Memory-Datenspeicher, der mehrere optimierte Caches bereithält. **Artikel werden 60 Tage (2 Monate) gespeichert** (max. 10.000), ältere werden automatisch entfernt.
    ...
    - `news_snapshot_pointer`: aktive Cache-Generation des Leseprotokolls
...
*   **Eine Generation, nicht drei:** Die drei Stufen werden nacheinander geholt und unabhängig voneinander am Edge gecacht. Damit daraus kein gemischter Stand entsteht, nennt jede Antwort ihre Cache-Generation. Die erste brauchbare Antwort legt sie fest, jede weitere wird daran gemessen: eine neuere Generation wird übernommen, eine ältere verworfen. Fehlt die Angabe – etwa in einem Cache von vor der Umstellung –, gilt das bisherige Verhalten. Einzelheiten: [Generationsgebundenes Leseprotokoll](docs/deployment/news-generations.md).
