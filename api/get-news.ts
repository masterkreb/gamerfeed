...
//
//                 if (!src || src.startsWith('data:') || src.includes('placeholder.svg') || src.includes('cpx.golem.de') || src.includes('feedburner.com')) {
//                     continue;
//                 }
//
...
function extractInitialData(item: any, feed: FeedSource): { imageUrl: string; needsScraping: boolean } {
    ...
    if (imageUrl) {
        try {
            ...
            // IMPORTANT: No specific rules for GameSpot or PC Games to fix regressions. They will pass through unmodified.
