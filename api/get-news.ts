...
//         let text = html;
//
//         // Remove common boilerplate text like "[...]" or "Weiterlesen..." BEFORE stripping tags
...
//             .replace(/<[^>]+>/g, '')
//             .replace(/\s+/g, ' ')
//             .replace(/\s*\.{3,}\s*$/, '') // Remove existing ellipses
//             .trim();
//
...
//     const isEmojiUrl = (url: string | undefined) => url && url.includes('s.w.org/images/core/emoji');
//
//     // More robust extraction order with immediate validation for each potential tag.
//     const mediaContentUrl = item['media:content']?.url;
//     if (mediaContentUrl && (item['media:content']?.medium === 'image' || item['media:content']?.type?.startsWith('image')) && !isEmojiUrl(mediaContentUrl)) {
//         imageUrl = mediaContentUrl;
//     }
//
//     const enclosureUrl = item.enclosure?.link;
//     if (!imageUrl && enclosureUrl && item.enclosure?.type?.startsWith('image') && !isEmojiUrl(enclosureUrl)) {
//         imageUrl = enclosureUrl;
//     }
//
//     const mediaThumbnailUrl = item['media:thumbnail']?.url;
//     if (!imageUrl && mediaThumbnailUrl && !isEmojiUrl(mediaThumbnailUrl)) {
//         imageUrl = mediaThumbnailUrl;
//     }
//
//     const thumbnailUrl = (item.thumbnail && typeof item.thumbnail === 'string') ? item.thumbnail : undefined;
//     if (!imageUrl && thumbnailUrl && !isEmojiUrl(thumbnailUrl)) {
//         imageUrl = thumbnailUrl;
//     }
//
//     // Fallback to parsing content if no valid primary image was found
//     if (!imageUrl) {
...
//
//             let mediaContentMatch = itemXml.match(/<media:content[^>]+url=["']([^"']+)["'][^>]*medium=["']image["']/i);
//             if (!mediaContentMatch) {
//                 mediaContentMatch = itemXml.match(/<media:content[^>]+url=["']([^"']+)["'][^>]*type=["']image\/[^"']+/i);
//             }
//
